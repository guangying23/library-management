...
class book
{
	...
	void createbook()
	{
		...
		cin>>bno;
		cout<<"\nENTER BOOK NAME";
		gets_s(bname);//enables enter with space
		cout<<"\nENTER AUTHOR NAME";
		gets_s(aname);
		cout<<"\n\n\nBook Created..";
	}
...
void writestudent()
{
 ...
 void displayspb(char n[])
 {
	 ...
	 {
		 if(_strcmpi(bk.retbno(),n)==0)//not case sensitive
		 {
			  bk.showbook();
			  flag=1;
		 }
 ...
 void displaysps(char n[])
 {
	 ...
	 {
		 if(_strcmpi(st.retadmno(),n)==0)//not case sensitive
		 {
			  st.showstudent();
			  flag=1;
		 }
 ...
 void modifystudent()
{
 ...
 void bookissue()
 {
	 ...
	 while(fp.read((char*)&st,sizeof(student))&& found==0)
	 {
		 if(_strcmpi(st.retadmno(),sn)==0)//compare admsn no.
		 {
			 found=1;
			 ...
			 cin>>bn;
			  ...
			  {
				   if(_strcmpi(bk.retbno(),bn)==0)//compare book no.
		             {
			          flag=1;
					  st.addtoken();
